export const normalizeReadingText = (text = '') =>
  String(text)
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
export const compareReadingText = (expectedText = '', spokenText = '') => {
  ...
  expectedWords.forEach((expectedWord, expectedIndex) => {
    ...
    spokenWords.forEach((spokenWord, spokenIndex) => {
      ...
      const score = Math.max(0, wordSimilarity(expectedWord, spokenWord) - Math.abs(spokenIndex - expectedIndex) * 0.03);
      if (score > best.score) best = { index: spokenIndex, score, word: spokenWord };
    });
    ...
    if (best.score >= 0.82) {
      ...
      if (best.score < 0.96) {
        pronunciationIssues.push({ expected: displayWord, spoken: best.word });
      }
      return;
    }
    ...
    if (best.word) incorrectWords.push({ expected: displayWord, spoken: best.word });
  });
...
export const splitIntoSentences = (text = '') => {
  ...
  const numberedItems = Array.from(
    normalizedText.matchAll(/(?:^|\s)(\d+)\.\s*([\s\S]*?)(?=(?:\s+\d+\.\s*)|$)/g),
    (match) => cleanReadingItem(match[2])
